...
func ProcesarFacturaXML(database *gorm.DB, invoice *Invoice, correoID uint) error {
	return database.Transaction(func(tx *gorm.DB) error {
		...
		doc := db.DocumentoComercial{
			...
			MonedaID:        moneda.ID,
			Subtotal:        invoice.LegalMonetaryTotal.TaxExclusiveAmount,
			Iva:             0, // Calcular o extraer del TaxTotal
		...
		if len(invoice.TaxTotal) > 0 {
			doc.Iva = invoice.TaxTotal[0].TaxAmount // Simplificación
		}
